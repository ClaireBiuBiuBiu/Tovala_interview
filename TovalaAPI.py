...
if __name__== "__main__":
    while True:
        try:
            print("Hey, this program contains extract news from API, load json file to s3 then load to snowflake table")
            user_action = input("Please tell me what do you want to do? Type: run or cleanup? ")
            if user_action == "run":
                Tovala = TovalaTools()
                ...
                break
            if user_action =='cleanup':
                Tovala = TovalaTools()
                Tovala.del_NewsAPI_S3()
                Tovala.conn_Snowflake()
                Tovala.sf_NewsAPI_deletion()
                print("Your workplace has been cleanup! See you next time")
                break
        except:
            print("Please tell me what do you want to do!!! Type: run or cleanup, or press ctrl+z to exit!!")
            continue
